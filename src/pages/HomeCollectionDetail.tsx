...
export function HomeCollectionDetail({ collections, allPortfolio, profile }: { collections: any[], allPortfolio: any[], profile: any }) {
  const navigate = useNavigate();
  const { id } = useParams();
  const selectedCollection = collections.find(c => c.id.toString() === id);
  ...
  const handleWorkClick = (work: any) => {
    navigate(`/home/post/${work.id}`);
  };

  if (!selectedCollection) {
    return (
      <div className="fixed inset-0 z-[80] bg-slate-50 flex items-center justify-center">
        <p className="text-slate-500">合集不存在</p>
        <button onClick={handleBack} className="mt-4 px-4 py-2 border rounded">返回</button>
      </div>
    );
  }
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className={cn("gap-3 space-y-3", selectedCollection.singleColumn ? "columns-1" : "columns-2")}>
              {selectedCollection.works?.map((workId: number) => {
                const work = allPortfolio.find(w => w.id === workId);
                if (!work) return null;
                return (
                  <motion.div 
                    key={work.id} 
